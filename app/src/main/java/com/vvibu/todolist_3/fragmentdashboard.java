...
//}
    ...
    //untuk mendeklarisasikan data yg di atas
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        rvTodo = v.findViewById(R.id.rvtodo);
        ...
        edtSearchBar.addTextChangedListener(new TextWatcher() {
            ...
            //menampilkan pencarian sesuai dengan kata yang diinput (sebelum di enter)
            @Override
            public void afterTextChanged(Editable s) {
                if (edtSearchBar.length() != 0) {
                    List<ToDo> searchlist = myDB.SearchData(edtSearchBar.getText().toString());
                    if (searchlist != null) {
                        rvTodo.setAdapter(new ToDoAdapter(getActivity(), searchlist));
                    }
                } else {
